...
| Backend real-model integration | `SFVS_REAL_MODELS=1 … pytest backend/tests/test_real_*.py` | **PASS** (re-run 2026-09-18: 12 passed; one Qwen load hit GPU OOM while another process held VRAM, then passed on retry) |
...
- **Tauri window, screenshots, keyboard/layout checks at 1280×800 and 1920×1080.** Layout walked in the
  ...
  Screenshots: `docs/screenshots/` (browser preview of the React shell, labelled PREVIEW MOCK). Native
  window chrome on Wayland may not be capturable with X11 tools. Install the `.deb` and walk
  New Voice → Create → Library at both sizes to confirm native window chrome.
- **Microphone recording with a real microphone.** Manual: Voices → New voice → Record with microphone →
  pick the device → Record 30 s → Stop; expect a 24-bit WAV, live meter, and the negotiated settings panel.
  Then unplug the device mid-recording; expect an error state with the partial file kept.
