...
export function makeApiModule(bus: EventBus) {
  const api = {
    ...
    system: {
      settingsGet: resolve({}),
      settingsSet: resolve({}),
      setOffline: resolve({ offline: false }),
      diagnostics: resolve({}),
      gpuStatus: resolve({ gpus: [] }),
      storageUsage: resolve({ data_dir: "/data", models_bytes: 0, recordings_bytes: 0, projects_bytes: 0, cache_bytes: 0, free_bytes: 0 }),
      clearCache: resolve({ freed_bytes: 0 }),
      logBundle: resolve({ path: "/data/exports/diagnostics.zip" }),
      cudaSmokeTest: resolve({ ok: true }),
    },
    audio: {
      import: resolve({}),
    ...
    speak: { session: resolve({ project_id: "proj_speak", text: "", script_version: 0, voice_id: null, engine_id: "qwen3-tts-base", language: "en", settings: {}, history: [] }), history: resolve({ history: [] }), remember: resolve({}), forget: resolve({ ok: true }) },
